    ...
    public  static Node getActivityDiagram(Document doc, TracerForm mainForm) {
        try {
            ...
            if (activityDiagramNL.getLength() == 0) {
                JOptionPane.showMessageDialog(null, "No activity diagrams were found on the package", "Warning", JOptionPane.INFORMATION_MESSAGE);
                return null;
            ...
            JOptionPane.showMessageDialog(null, e.getMessage() + "\n" + e.toString(), "Exception in main", JOptionPane.INFORMATION_MESSAGE);
            return null;
        }
    }
    ...
    public static void collectNodes(
        ...
        try {
            ...
            if (diagramElementIDsNL.getLength() == 0) {
                JOptionPane.showMessageDialog(null, "No elements were found in diagram.", "Warning", JOptionPane.INFORMATION_MESSAGE);
                return;
            }
            ...
            for(int i = 0; i < diagramElementIDsNL.getLength(); i++) {
                ...
                if (elementNL .getLength() == 0) {
                    JOptionPane.showMessageDialog(null, "ID in diagram not found on the package. ID: " + id +
                            "\nDoes this diagram contain elements form another package pasted as a link?", "Warning", JOptionPane.WARNING_MESSAGE);
                    continue;
                } else  if (elementNL.getLength() > 1) {
                    JOptionPane.showMessageDialog(null, "More than one element was found on the package with the following ID: ." + id, "Warning", JOptionPane.WARNING_MESSAGE);
                    continue;
                }
            ...
            for (Map.Entry<String, Node> nodeEntry : nodeNodeMap.entrySet()) {
                ...
                if(type == null) {
                    JOptionPane.showMessageDialog(null, "Couldn't get the node type for node ID " + id, "Warning", JOptionPane.INFORMATION_MESSAGE);
                    break;
                }
            ...
            JOptionPane.showMessageDialog(null, e.getMessage() + "\n" + e.toString(), "Exception:", JOptionPane.INFORMATION_MESSAGE);
        }
    }
    ...
    private static String getActNodeType(Node node, String id) {
        ...
        try {
            ...
            if (stypeNL.getLength() == 0) {
                JOptionPane.showMessageDialog(null, "No stype found for pseudostate node, " + id, "Warning", JOptionPane.INFORMATION_MESSAGE);
                return null;
            } else if (stypeNL.getLength() > 1) {
                JOptionPane.showMessageDialog(null, "More than one stype was found for pseudostate node with the following ID: " + id, "Warning", JOptionPane.INFORMATION_MESSAGE);
                return null;
            }
            ...
            if(stype.equals("StateNode")) {
                try {
                    ...
                    if (ntypeNL.getLength() == 0) {
                        JOptionPane.showMessageDialog(null, "No ntype found for StateNode, " + id, "Warning", JOptionPane.INFORMATION_MESSAGE);
                        return null;
                    } else if (ntypeNL.getLength() > 1) {
                        JOptionPane.showMessageDialog(null, "More than one ntype was found for StateNode with the following ID: " + id, "Warning", JOptionPane.INFORMATION_MESSAGE);
                        return null;
                    }
